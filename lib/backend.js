                ...
                if (activeOrder) {
                    if (market.marketType !== 'dynamic') {
                        ...
                        if (orders.buy[mineral]) {
                            ...
                            // Get saturation point based off terminal amount and rarity
                            let saturationPoint = market.saturationPoint * Math.ceil((playerTerminals.length + 2.5) * 0.25)
                            if (commodityTiers[0].includes(mineral)) {
                                saturationPoint *= 0.9
                            } else if (commodityTiers[1].includes(mineral)) {
                                saturationPoint *= 0.725
                            ...
                            // Add one to total stockpile to avoid divide by zero
                            let saturationModifier = (saturationPoint / (totalStockpile + 1));
                            // Max multi is 3
                            if (saturationModifier > 3) saturationModifier = 3;
                            order.remainingAmount = market.buyAmount * (playerTerminals.length + 1)
                            order.totalAmount = market.buyAmount * (playerTerminals.length + 1)
                            order.amount = market.buyAmount * (playerTerminals.length + 1)
                        ...
                        if (orders.sell[mineral]) {
                            ...
                            order.amount = market.sellAmount * (playerTerminals.length + 1)
                            // Adjust price based on saturation point
                            let saturationModifier = 1
                            // If saturationPoint is met increase price to simulate lack of supply
                            if (totalSold > saturationPoint) {
                                // Add one to total sold to avoid divide by zero
                                saturationModifier = ((totalSold + 1) / saturationPoint);
                                // Max multi is 3
                                if (saturationModifier > 3) saturationModifier = 3;
                            }
