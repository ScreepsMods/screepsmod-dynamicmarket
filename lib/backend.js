...
async function updateOrders(config) {
    ...
    let ps = NPCTerminals.map(async terminal => {
        ...
        if (!marketData || marketData['nextUpdate'] < gameTime) {
            ...
            delete terminal._id
            await db['rooms.objects'].update({room: terminal.room, type: 'terminal'}, {$set: terminal})
        }
        // Update orders
        queued.forEach(o => delete o._id)
        queued.forEach((o) => db['market.orders'].update({type: o.type, roomName: o.roomName, resourceType: o.resourceType}, {$set: o}))
    })
    return Promise.all(ps)
}
