                ...
                if (activeOrder) {
                    if (market.marketType !== 'dynamic') {
                        ...
                        if (orders.buy[mineral]) {
                            ...
                            // If price is above the lowest sell order, adjust it (no flipping for profit to npc)
                            let lowestSellOrder = db['market.orders'].find({$and: [{resourceType: mineral}, {type: 'sell'}, {remainingAmount: {$gt: 1}}]}).sort(function(a, b) {return a.price - b.price;})[0]
                            if (lowestSellOrder && order.price > lowestSellOrder.price) order.price = lowestSellOrder.price * 0.98;
