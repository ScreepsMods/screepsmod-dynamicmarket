...
async function updateOrders(config) {
    ...
    let ps = NPCTerminals.map(async terminal => {
        ...
        if (!marketData || marketData['nextUpdate'] < gameTime) {
            ...
            // Get the lowest sell order for every resource
            let lowestOrder = {};
            let sellOrders = await db['market.orders'].find({$and: [{type: 'sell'}, {remainingAmount: {$gt: 1}}]});
            for (let order of sellOrders) {
                if (!lowestOrder[order.resourceType] || lowestOrder[order.resourceType] > order.price) lowestOrder[order.resourceType] = order.price;
            }
            let mineralOrders = arrayUnique(Object.keys(market.buyPrices).concat(Object.keys(market.sellPrices)))
            mineralOrders.forEach(mineral => {
                ...
                if (activeOrder) {
                    if (market.marketType !== 'dynamic') {
                        ...
                        if (orders.buy[mineral]) {
                            ...
                            let saturationPoint = market.saturationPoint * Math.ceil((playerTerminals.length + 2.5) * 0.5)
                            if (commodityTiers[0].includes(mineral)) {
                                saturationPoint = (market.saturationPoint * (Math.random() * (+0.95 - +0.85) + +0.85)) * Math.ceil((playerTerminals.length + 2.5) * 0.5)
                            } else if (commodityTiers[1].includes(mineral)) {
                                saturationPoint = (market.saturationPoint * (Math.random() * (+0.8 - +0.65) + +0.65)) * Math.ceil((playerTerminals.length + 2.5) * 0.5)
                            } else if (commodityTiers[2].includes(mineral)) {
                                saturationPoint = (market.saturationPoint * (Math.random() * (+0.6 - +0.35) + +0.35)) * Math.ceil((playerTerminals.length + 2.5) * 0.5)
                            } else if (commodityTiers[3].includes(mineral)) {
                                saturationPoint = (market.saturationPoint * (Math.random() * (+0.4 - +0.2) + +0.2)) * Math.ceil((playerTerminals.length + 2.5) * 0.5)
                            } else if (commodityTiers[4].includes(mineral)) {
                                saturationPoint = (market.saturationPoint * (Math.random() * (+0.25 - +0.05) + +0.05)) * Math.ceil((playerTerminals.length + 2.5) * 0.5)
                            }
                            // Use some of the stockpile every trade cycle
                            if (resourceData['stockpileUseTick'] < gameTime) {
                            ...
                            // Check if a sell order exists, and make sure the buy order does not exceed it
                            if (lowestOrder[mineral] && order.price > lowestOrder[mineral]) order.price = lowestOrder[mineral] * 0.98;
            ...
            delete terminal._id
            await db['rooms.objects'].update({_id: terminalID}, {$set: terminal})
        }
        // Update orders
        queued.forEach(({ _id, ...order }) => db['market.orders'].update({ _id }, { $set: order }))
    })
    return Promise.all(ps)
}
